...
public class Semester_8_cse_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_semester_8_cse_);

        firebaseFirestore = FirebaseFirestore.getInstance();
        mFirestoreList = findViewById(R.id.sem_8_recycler);

        //Query
        Query query = firebaseFirestore.collection("SEMESTER_8_CSE");
        ...
        adapter = new FirestoreRecyclerAdapter<firestore_examDetails_access, Semester_8_cse_Activity.ProductsViewHolder>(options) {
            @NonNull
            @Override
            public Semester_8_cse_Activity.ProductsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.examdetails_list, parent, false);

                return new Semester_8_cse_Activity.ProductsViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(@NonNull Semester_8_cse_Activity.ProductsViewHolder holder, int position, @NonNull firestore_examDetails_access model) {
