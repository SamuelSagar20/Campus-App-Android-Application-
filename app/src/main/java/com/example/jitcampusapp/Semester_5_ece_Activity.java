...
public class Semester_5_ece_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_semester_5_ece_);

        firebaseFirestore = FirebaseFirestore.getInstance();
        mFirestoreList = findViewById(R.id.sem_5_recycler);

        //Query
        Query query = firebaseFirestore.collection("SEMESTER_5_ECE");
        ...
        adapter = new FirestoreRecyclerAdapter<firestore_examDetails_access, Semester_5_ece_Activity.ProductsViewHolder>(options) {
            @NonNull
            @Override
            public Semester_5_ece_Activity.ProductsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.examdetails_list, parent, false);

                return new Semester_5_ece_Activity.ProductsViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(@NonNull Semester_5_ece_Activity.ProductsViewHolder holder, int position, @NonNull firestore_examDetails_access model) {
