...
public class Semester_6_eee_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_semester_6_eee_);

        firebaseFirestore = FirebaseFirestore.getInstance();
        mFirestoreList = findViewById(R.id.sem_6_recycler);

        //Query
        Query query = firebaseFirestore.collection("SEMESTER_6_EEE");
        ...
        adapter = new FirestoreRecyclerAdapter<firestore_examDetails_access, Semester_6_eee_Activity.ProductsViewHolder>(options) {
            @NonNull
            @Override
            public Semester_6_eee_Activity.ProductsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.examdetails_list, parent, false);

                return new Semester_6_eee_Activity.ProductsViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(@NonNull Semester_6_eee_Activity.ProductsViewHolder holder, int position, @NonNull firestore_examDetails_access model) {
