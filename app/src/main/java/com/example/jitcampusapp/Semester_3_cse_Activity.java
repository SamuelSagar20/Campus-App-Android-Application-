...
public class Semester_3_cse_Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_semester_3_cse_);

        firebaseFirestore = FirebaseFirestore.getInstance();
        mFirestoreList = findViewById(R.id.sem_3_recycler);

        //Query
        Query query = firebaseFirestore.collection("SEMESTER_3_CSE");
        ...
        adapter = new FirestoreRecyclerAdapter<firestore_examDetails_access, Semester_3_cse_Activity.ProductsViewHolder>(options) {
            @NonNull
            @Override
            public Semester_3_cse_Activity.ProductsViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.examdetails_list, parent, false);

                return new Semester_3_cse_Activity.ProductsViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(@NonNull Semester_3_cse_Activity.ProductsViewHolder holder, int position, @NonNull firestore_examDetails_access model) {
